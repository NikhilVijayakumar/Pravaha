...
from pravaha.domain.workflow.repository.json_workflow_config_repository import JsonWorkflowConfigRepository
...
class LocalWorkflowManager:
    def __init__(
        ...
        config_repository: Optional[WorkflowConfigRepositoryProtocol] = None
    ):
        ...
        if defaults:
            ...
            # Use cache_root for default workflow paths
            cache_root = cache_config.cache_root
            self.defaults = {
                "details": str(cache_root / "workflow" / "details"),
                "run": str(cache_root / "workflow" / "run")
            }
